...
import com.example.healthystate.Models.FoodModel
...
class AddFoodDialog : AppCompatActivity() {
    ...
    lateinit var discript: EditText
    var countgramsint: Int = 0
    var countcaloriesint: Int = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.addfood_window)
        ...
        discript = findViewById(R.id.edtDescription)
        ...
        val addBtn = findViewById<Button>(R.id.AddButFood)
        addBtn.setOnClickListener{
            countgramsint = countgrams.getText().toString().trim().toInt();
            countcaloriesint = countcalories.getText().toString().trim().toInt();


            val obj = FoodModel(namefood.text.toString().trim(), countgramsint, countcaloriesint, discript.text.toString().trim(), dateFood.text.toString().trim(), paperDbClass.getUser()!!.idLogPass,
            ...
            retrofit.createFood(obj)
                .enqueue(object: Callback<DefaultResponse> {
                    ...
                    override fun onResponse(call: Call<DefaultResponse>, response: Response<DefaultResponse>) {
                       // Toast.makeText(applicationContext,  spinner.selectedItemId.toInt().toString(), Toast.LENGTH_LONG).show()
                        val intent: Intent =
